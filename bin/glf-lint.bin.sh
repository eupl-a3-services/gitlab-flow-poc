...
glf_lint() {
    ...
    log INFO "Current semver: $CURRENT_SEMVER"
    log INFO "Last known semver: $GLF_SEMVER"

    if [[ "$CURRENT_SEMVER" == "$GLF_SEMVER" ]]; then
    log SUCCESS "Version is up to date."
    exit 0
    fi
    ...
    log ERROR "Newer version is available: $GLF_SEMVER"
    exit 1
    else
    log INFO "Updating tracked version to $CURRENT_SEMVER"
