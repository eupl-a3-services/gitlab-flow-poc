...
log_level() {
    ...
    case "${LEVEL}" in
        HEAD|DEBUG|INFO|WARN|ERROR|SUCCESS)
            export GLAB_LOG_LEVEL="${LEVEL}"
            echo -e "\033[1;36m[INFO] Log level is set to: ${GLAB_LOG_LEVEL}\033[0m"
            ;;
        *)
...
log_info() {
    if [ "${GLAB_LOG_LEVEL}" = "INFO" ] || [ "${GLAB_LOG_LEVEL}" = "DEBUG" ]; then
