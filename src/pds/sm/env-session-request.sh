...
env_session_request(){
    ...
    local TARGET_DIR="/secure-storage/env"
    local EXTENSION=".env-session-request"

    COUNT=$(find "$REQUEST_DIR" -type f -name "*${EXTENSION}" | wc -l)

    if [ "${COUNT}" -eq 0 ]; then
        printc CYAN "INFO" "No pending env session requests found."
        exit 0
    fi
    ...
    local UNZIP_TARGET="${TARGET_DIR}/${BASE_NAME%${EXTENSION}}.env"
